...
### 🥇 最優先: Design Token Documentation
...
Phase 3で確立した二層変数アーキテクチャの詳細ドキュメントを作成します。

**作成するドキュメント:**

1. **アーキテクチャガイド** (`docs/architecture.md`)
   - 二層変数システムの仕組み
   - トークン参照の階層構造
   - テーマ切り替えの実装方法
   - CSS Custom Propertiesの活用パターン

2. **使用方法ガイド** (`docs/usage-guide.md`)
   - セマンティックトークンの使い方
   - コンポーネントトークンの参照方法
   - テーマカスタマイズガイド
   - ベストプラクティス

3. **コンポーネントリファレンス** (`docs/components/`)
   - 各コンポーネントの利用可能なトークン一覧
   - バリアント別のトークン参照
   - 状態別トークンの説明

4. **移行ガイド** (`docs/migration-guide.md`)
   - 旧構造から新構造への移行手順
   - 破壊的変更の詳細
   - 移行チェックリスト

**期待される成果:**
- 開発者が二層変数システムを理解し活用できる
- 新規コントリビューターがトークンを適切に追加できる
- テーマカスタマイズが容易になる

---

### 🥈 優先度高: Component Usage Examples
...
実装済みの9コンポーネントの実践的な使用例を作成します。

**作成する例:**

1. **基本使用例** (`examples/basic/`)
   - Button - 4バリアント × 各状態
   - Input - バリデーション状態
   - Card - ホバー/選択状態
   - Badge - 4種類のフィードバック
...
**最優先事項:**
1. ✅ リファクタリング計画ドキュメントの更新 (進行中)
2. ⏳ Design Token Documentationの作成
3. ⏳ Component Usage Examplesの実装
4. ⏳ 追加コンポーネントの検討

**推定工数 (Phase 4):**
- ドキュメント作成: 8-12時間
- 使用例作成: 6-8時間
- 追加コンポーネント: 20-30時間 (オプション)

---

## 付録: バンドルサイズパフォーマンス

Phase 3完了時点でのバンドルサイズ目標:

```javascript
const SIZE_BUDGETS = {
  'js/index.js': {
    raw: 55 * 1024,      // 55 KB (9コンポーネント対応)
    gzip: 15 * 1024,     // 15 KB
  },
  'css/variables.css': {
    raw: 100 * 1024,     // 100 KB
