...
export const ProgressSteps: Story = {
  render: () => {
    ...
    steps.forEach((step) => {
      ...
      progress.setAttribute('show-label', '');
      progress.setAttribute('color', step.value === 100 ? 'success' : 'primary');
...
export const MultipleProgress: Story = {
  render: () => {
    ...
    const tasks = [
      { label: 'Installing dependencies', value: 100, color: 'success' as const },
    ...
    tasks.forEach((task) => {
      ...
      progress.setAttribute('show-label', '');
      progress.setAttribute('variant', task.value > 0 && task.value < 100 ? 'animated' : 'default');
