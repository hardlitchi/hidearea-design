...
Hidearea Designで利用可能な全31コンポーネントの概要です。
...
### DataGrid ✨ NEW
高度なデータグリッド。ソート、ページネーション、行選択機能を備えた大量データ表示に最適。

[詳細を見る →](/components/datagrid)
...
### データ表示
1. **Table** - シンプルなデータテーブル
2. **DataGrid** - 高度なデータグリッド（ソート・ページネーション・選択）
3. **List** - リスト表示
4. **Accordion** - 折りたたみ可能なコンテンツ
5. **Drawer** - サイドパネル
...
全31コンポーネントの実装が完了しました。

### 今後の拡張機能（検討中）
- DataGrid: カラムリサイズ・並び替え、フィルタリング、編集可能なセル
- 仮想スクロール対応
- テーマカスタマイズツール
- アニメーションライブラリ
